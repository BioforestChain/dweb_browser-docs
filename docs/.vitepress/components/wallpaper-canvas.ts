...
const calcMatrixScaleX = (matrix: DOMMatrix) =>
  Math.sqrt(matrix.a * matrix.a + matrix.c * matrix.c);
const calcMatrixScaleY = (matrix: DOMMatrix) =>
  Math.sqrt(matrix.b * matrix.b + matrix.d * matrix.d);

const keyframesToTransform = <T extends { offset: number }>(
  keyframes: T[],
  effect: (
    origin: CanvasRectAttributes,
    start: T,
    end: T,
    progress: number
  ) => CanvasRectAttributes
) => {
  ...
  return (origin: CanvasRectAttributes, fraction: number) => {
    ...
    return effect(
      ...
      start,
        ...
        ? 1
        : (fraction - start.offset) / (end.offset - start.offset)
    );
  };
};
...
class CanvasAnimation {
  ...
  cancel() {
    this.target.animations = this.target.animations.filter(
      (ani) => ani !== this
    );
  }
  calcEffect(
    attrs: CanvasRectAttributes,
    aniDuration = this.playDuration + this.iterationStart
  ) {
  ...
  calc(attrs: CanvasRectAttributes, now: number = performance.now()) {
    ...
    this.playDuration = Math.min(
      this.playDuration + frameDuration,
      maxPlayDuration
    );
...
export class DwebWallpaperElement extends HTMLElement {
  ...
  doAni(rectEles: CanvasRectElement[]) {
    const anis = rectEles.map((ele, index) =>
      this.aniRect(ele, index, rectEles.length)
    );
  ...
  replay(
    ...
    return new Promise<void>((resolve) => {
      ...
      const down = (currentTime: number) => {
        ...
        const playbackRate =
          startPlaybackRate +
          easing(progress) * (endPlaybackRate - startPlaybackRate);
  ...
  #calcMixBlendModeMap() {
    ...
    return mixBlendModeMap.map((mode) => {
      return (mode.split(/[\s,]+/).filter((mode) => /^\w/.test(mode))[0] ||
        "hard-light") as GlobalCompositeOperation;
    });
  }

  #calcColorsMap() {
    const colorsMap = Array.from(
      { length: 24 },
  ...
  doInit() {
    ...
    const background = `linear-gradient(var(--bg-img-deg, 0deg), ${colors
      .map((rgb, i) => `var(--bg-img-color-${i}, rgba(${rgb},1))`)
      .join(", ")})`;
      ...
      return new CanvasRectElement(
        new CanvasRectAttributes(1, 1, {
          ...
          stopColors: [
            ...
            {
              color: `rgba(${rgb.map((c) =>
                Math.min(Math.max(0, c + rand(-0, 20)))
              )},0.0)`,
              offset: 1,
            },
          ],
  ...
  aniRect(rect: CanvasRectElement, index: number, length: number) {
    ...
    const startRect = rect.animations
      .find((ani) => ani.name == "ani-rect")
      ?.calcEffect(rect.attrs);
    const startScaleX = startRect
      ? calcMatrixScaleX(startRect.transform)
      : scaleX();
    const startScaleY = startRect
      ? calcMatrixScaleY(startRect.transform)
      : scaleY();
    const startRotate = startRect
      ? calcMatrixAngle(startRect.transform)
      : rand(0, 360);
    const startRectTransformOriginX = startRect
      ? startRect.transformOriginX
      : rand(0.4, 0.6);
    const startRectTransformOriginY = startRect
      ? startRect.transformOriginY
      : rand(0.4, 0.6);
    ...
    const keyframes = randomArray(rand(3, 10), (frame) => {
      ...
      return {
        ...
        scaleY: scaleY(),
        transformOriginX:
          (absMax(frame.offset, 0.5) / 10) * rand() + startRectTransformOriginX,
        transformOriginY:
          (absMax(frame.offset, 0.5) / 10) * rand() + startRectTransformOriginY,
      };
    });
    ...
    const ani = new CanvasAnimation(
      ...
      keyframesToTransform(keyframes, (origin, start, end, progress) => {
        ...
        return origin.copy({
          transform: transform,
          transformOriginX: calcNumberAnimation(
            start.transformOriginX,
            end.transformOriginX,
            progress
          ),
          transformOriginY: calcNumberAnimation(
            start.transformOriginY,
            end.transformOriginY,
            progress
          ),
        });
      }),
...
const randomArray = <R>(
  len: number,
  map: (ele: { offset: number }, index: number) => R
) =>
  Array.from({ length: (len = Math.round(len)) }, (_, index) =>
    map({ offset: index / (len - 1) }, len)
  );
const rand = (min = -1, max = 1) => Math.random() * (max - min) + min;
const absMax = (val: number, max: number) => max - Math.abs(max - val);

customElements.get("dweb-wallpaper") ||
